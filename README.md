...
- **¿Qué genera?**
  ...
  - Podrás encontrar el resultado en formato `Reporte_Estrategico_Rappi.md` (Markdown) y `Reporte_Estrategico_Rappi.pdf` (PDF) dentro de la carpeta `src/insights/`.
...
El flujo de conversación en `src/bot/agent.py` sigue un diseño guiado por estados (`StateGraph`) estructurado en 5 nodos principales:
...
Este módulo no espera preguntas, su objetivo es analizar activamente la base de datos de inicio a fin para producir un reporte gerencial. Su ejecución en `src/insights/main.py` y `src/insights/tools.py` sigue un diseño ReAct cerrado con auditoría (Reflexión):
...
El Bot inyecta su propio `CallbackHandler` de Langfuse por sesión para registrar las consultas en la interfaz (`src/core/observability.py`), mientras que el Pipeline de Insights tiene su propio entorno (separado con el prefijo `_INSIGHTS_`) para registrar la generación masiva de análisis e inferencias (`src/insights/main.py`).
